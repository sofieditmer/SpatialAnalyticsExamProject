...
So, our hypothesis is that the spatial regression will fit better on our data (lower AIC score) compared to our global model. Furthermore, we hypothesize that the global model residuals will show spatial clustering whereas the spatial model residuals will show random dispersion. <br>
...
We read in the data from the data folder and change the column names to more intuitive names as these have been abbreviated by the st_write function. <br>
We also normalize the dependent variables that we will use in our regression to bring all values to the same range without distorting differences in the ranges of values.
<br>
```{r}
# reading the data
...
constituency_data$perc_votes_conservative <- normalize(constituency_data$perc_votes_conservative)
constituency_data$perc_votes_labour <- normalize(constituency_data$perc_votes_labour)
...
```{r}
# Define neighbor list (Queen's adjacency neighborhood definition)
...
```
<br>
For the model on labour votes, the autocorrelation test points to a spatial clustering of the model residuals.
<br>
...
In geographically weighted regression (GWR), we sort of compromise between a global model (a model across all points in space) and a local model (a model only containing one observation). This is done by modeling across a subset of the data through a bandwidth kernel and weighting the different observations according to how close they are in space to the focal observation. So, we make a regression for each location (constituency) and the model estimates then depend on the location of the observations. This allows us to discern the importance of our predictors at different constituencies in England in relation to how they affect voting patterns. <br>

The bandwidth will be what bias the model in terms of defining the neighbors. Essentially, finding the bandwidth is like asking how many locations should we use to compute the varying slope as we more though the data space? We can choose either that the bandwidth is a given diameter around our focal point or it can be adaptive so that it takes the same number of observations for each focal points. The latter would mean that for denser areas, the bandwidth would be smaller. As our areas vary a lot in density, we choose the latter. <br>
...
As we are using an sf object, we also have to define a set of coordinates for each constituency so we first define the centroids for each constituency. Then, we find the optimal bandwidth for our model on conservative votes. Lastly, we use this to fit the model to the observations. 
...
# fitting the model for conservative votes
...
```
Here, we see that we get an optimal bandwidth of 7 neighbors. Using this for our model, we improve the adjusted R2 from 57% for the global regression model to 91% for the GWR model. Similarly, the AICc value decreases from 1067.75 for the global regression model to 404.35 for the GWR model.
...
map1 <- tm_shape(gwr_conservative_merged) + 
  tm_fill("perc_white_pop.1",
          n = 5,
          style = "quantile",
          title = "Coefficient for percent white population")  +
  tm_layout(frame = FALSE,
            main.title = "A)")
...
map2 <- tm_shape(gwr_conservative_merged) + 
  tm_fill("pubs_per_inhabitant.1",
          n = 5,
          style = "quantile",
          title = "Coefficient for pubs per inhabitant")  +
  tm_layout(frame = FALSE,
            main.title = "B)")
...
map3 <- tm_shape(gwr_conservative_merged) + 
  tm_fill("pop_density.1",
          n = 5,
          style = "quantile",
          title = "Coefficient for population density")  +
  tm_layout(frame = FALSE,
            main.title = "C)")
...
map4 <- tm_shape(gwr_conservative_merged) + 
  tm_fill("avg_age.1",
          n = 5,
          style = "quantile",
          title = "Coefficient for average age")  +
  tm_layout(frame = FALSE,
            main.title = "D)")
...
map5 <- tm_shape(gwr_conservative_merged) + 
  tm_fill("avg_income.1",
          n = 5,
          style = "quantile",
          title = "Coefficient for average income")  +
  tm_layout(frame = FALSE,
            main.title = "E)")
...
Here, we see that the spatial clustering is still significant but also remarkably less than for the global model. Moran's I for the GWR for Conservative votes is 0.13 compared to 0.60 for the global model. However, we should remain cautious when drawing inferences on the basis of the autocorrelation test for the GWR as this is using a different optimization approach.
<br>


__GWR for Labour Votes:__ <br>
Then, we define a similar GWR for our model on labour votes
<br>
```{r}
# calculate Optimal kernel bandwidth using the constituency coordinates defined when specifying neighborhoods, and adapting the kernel size according to density of areas
...
# fitting the model for conservative votes
gwr_model_labour = gwr(perc_votes_labour ~ pop_density + pubs_per_inhabitant + perc_white_pop + avg_age + avg_income, data = constituency_data, coords = constituency_centroids, adapt=bw, hatmatrix=TRUE, se.fit=TRUE, gweight=gwr.Gauss) 
...
```
<br>
Plot bandwidth distances for each constituencies for each model. <br>
```{r}
